...
public final class DebugStringsKt {
    ...
    public static final String toDebugString(Continuation<?> continuation) {
        String m115constructorimpl;
        if (continuation instanceof DispatchedContinuation) {
            return continuation.toString();
        }
        try {
            Result.Companion companion = Result.Companion;
            m115constructorimpl = Result.m115constructorimpl(continuation + '@' + getHexAddress(continuation));
        } catch (Throwable th) {
        ...
        if (Result.m118exceptionOrNullimpl(m115constructorimpl) != null) {
            m115constructorimpl = continuation.getClass().getName() + '@' + getHexAddress(continuation);
        }
